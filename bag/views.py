    ...
    if size:
        if item_id in list(bag.keys()):
            if size in bag[item_id]["items_by_size"].keys():
                ...
                messages.success(
                    request,
                    f"Updated {product.name} (size: {size.upper()}) quantity to {bag[item_id]['items_by_size'][size]}!",
                )
            else:
                bag[item_id]["items_by_size"][size] = quantity
                messages.success(
...
def adjust_bag(request, item_id):
    """A view to adjust the quantity of the specified product to the specified amount in the shopping bag"""
    product = get_object_or_404(Product, pk=item_id)
    quantity = int(request.POST.get("quantity"))
    size = None
    if "product_size" in request.POST:
...
def remove_from_bag(request, item_id):
    """A view to remove the specified product from the shopping bag"""
    try:
        product = get_object_or_404(Product, pk=item_id)
        size = None
        if "product_size" in request.POST:
